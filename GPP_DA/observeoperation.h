	   	...
	   	if(size_t(mpi_rank % mpic.size) == (i % mpic.size)){
			...
			sprintf(cmd_str, "mkdir %s/ensemble-%.3lu", fc.DA_dir, i );
		 	...
		 	/*write ssrd file*/	
               		...
               		sprintf(ssrd_str, "%s/ssrd/ec-ei-fc012up2tr3-sfc-glb100x100-ssrd_%d%.2lu%.2lu_00p03.hdf.%d.trans.tif", fc.in_dir, DA_itr.year, DA_itr.month, DA_itr.day, DA_itr.hour);
			infs.push_back(ssrd_str);
			
		 	/*write d2m file*/	
               		char d2m_str[128];//string buffer of command 
               		sprintf(d2m_str, "%s/d2m/ec-ei-fc012up2tr3-sfc-glb100x100-d2m_%d%.2lu%.2lu_00p03.hdf.%d.trans.tif", fc.in_dir, DA_itr.year, DA_itr.month, DA_itr.day, DA_itr.hour);
			infs.push_back(d2m_str);
			
		 	/*write Tmp file*/	
               		char Tmp_str[128];//string buffer of command 
               		sprintf(Tmp_str, "%s/t2m/ec-ei-fc012up2tr3-sfc-glb100x100-t2m_%d%.2lu%.2lu_00p03.hdf.%d.trans.tif", fc.in_dir, DA_itr.year, DA_itr.month, DA_itr.day, DA_itr.hour);
